 ...
 */
function extractSession(lines, options = { maxChars: 500000, toolUseMaxChars: 200 }) {
    const chunks = [];
    let charCount = 0;
    let totalEntries = 0;
    let keptEntries = 0;
 ...
 */
function formatEntry(entry, toolUseMaxChars) {
 ...
 */
function formatContent(content, toolUseMaxChars) {
...
// --- CLI ---
function printUsage() {
    process.stderr.write('Usage: extract-session <session.jsonl> [--output <path>] [--max-chars <n>]\n' +
...
function main(args = process.argv.slice(2)) {
    ...
    if (outputPath) {
        (0, fs_1.writeFileSync)(outputPath, fullOutput, 'utf-8');
        process.stderr.write(`Extracted ${result.keptEntries}/${result.totalEntries} entries → ${outputPath}\n`);
    }
    else {
